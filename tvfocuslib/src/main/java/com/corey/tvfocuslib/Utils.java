...
public final class Utils {
	 ...
	 * size; 2) Disable hardware acceleration. Use the 2nd solution here.
	 */
	...
	public static boolean hasShortCut(Context context, String shortCutName) {
		...
		Cursor cursor = resolver.query(Uri.parse(url), null, "title=? and iconPackage=?", new String[]{shortCutName, context.getPackageName()},
				null);

		if (null == cursor || !cursor.moveToFirst()) {
			url = "content://com.android.launcher2.settings/favorites?notify=true";
			cursor = resolver.query(Uri.parse(url), null, "title=? and iconPackage=?", new String[]{shortCutName, context.getPackageName()}, null);
		}
	...
	public static int sp2px(float spValue) {
		final float fontScale = ContextProvider.getApplicationContext().getResources().getDisplayMetrics().scaledDensity;
	...
	public static int dip2px(Context paramContext, float paramFloat) {
		...
		return (int) Math.ceil(paramFloat * localDisplayMetrics.density);
	}
	 ...
	 *
	 * @param context
	 * @param startClass   快捷方式启动的界面。
	 * @param shortCutName 快捷方式名称。
	 */
	public static void delShortCut(Context context, Class<?> startClass, String shortCutName) {
		Intent shortcut = new Intent("com.android.launcher.action.UNINSTALL_SHORTCUT");
		shortcut.putExtra(Intent.EXTRA_SHORTCUT_NAME, shortCutName);

		// 不能用intent.setAction("xxx").setComponent来设置intent。这样会有手机不能启动。
		Intent startIntent = new Intent(context, startClass).setAction("android.intent.action.MAIN").addCategory("android.intent.category.LAUNCHER");
	...
	/**
	 * @param context
	 * @param startClass    要开启的界面的class
	 * @param shortCutName  快捷方式名称
	 * @param shortCutResId 快捷方式图标的资源id。
	 * @param coverOld      有快捷方式的情况下，coverOld == true，覆盖原快捷方式；coverOld == false，取消创建；
	 */
	public static void addShortcut(Context context, Class<?> startClass, String shortCutName, int shortCutResId, boolean coverOld) {
		...
		// 不能用intent.setAction("xxx").setComponent来设置intent。这样会有手机不能启动。
		shortcut.putExtra(Intent.EXTRA_SHORTCUT_INTENT, startIntent);
		ShortcutIconResource iconRes = ShortcutIconResource.fromContext(context, shortCutResId);
	...
	public static List<String> findImageUrls(String rawData) {
		...
		for (Iterator<String> iterator = imgUrls.iterator(); iterator.hasNext(); ) {
			resultUrls.add(iterator.next().replaceAll("<img.*?src=\"", "").replaceAll("\".*", ""));
		}
		return resultUrls;
	}
	 ...
	 * scaled. If the specified file name is null, or cannot be decoded into a
	...
	public static Bitmap decodeBitmap(String pathName) {
		...
		try {
			...
			opts.inSampleSize = computeSampleSize(opts, UNCONSTRAINED, MAX_NUM_PIXELS);
	...
	private static int computeInitialSampleSize(BitmapFactory.Options options, int minSideLength, int maxNumOfPixels) {
		...
		if (w > MAX_IMAGE_SIZE || h > MAX_IMAGE_SIZE) {
			int scale = (int) Math.ceil(Math.max(w / MAX_IMAGE_SIZE, h / MAX_IMAGE_SIZE));
	...
	public static String base64Encode(String str) {
		return Base64.encodeToString(str.getBytes(), Base64.NO_PADDING | Base64.NO_WRAP | Base64.URL_SAFE);
	}
	 ...
	 * 11+, so use reflection instead.
	 *
	 * @param view
	 * @param renderMode 渲染模式。{@link #LAYER_TYPE_NONE}, {@link #LAYER_TYPE_SOFTWARE},
	 *                   {@link #LAYER_TYPE_HARDWARE}其中一个。
	 */
	public static void adjustRenderMode(View view, int renderMode) {
		try {
			// View.setLayerType
			Method method = View.class.getMethod(LAYER_TYPE_METHOD, int.class, Paint.class);
	 ...
	 * given array empty an empty string will be returned. Null elements of the
